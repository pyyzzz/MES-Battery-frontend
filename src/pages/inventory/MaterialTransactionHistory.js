...
const MATERIALS = [
  ["MAT-20260209-0001", "납(Pb)", "KG"],
  ["MAT-20260209-0002", "양극판", "EA"],
  ["MAT-20260209-0003", "음극판", "EA"],
  ["MAT-20260209-0004", "분리판", "EA"],
  ["MAT-20260209-0005", "전해액", "L"],
  ["MAT-20260209-0006", "케이스", "EA"],
  ["MAT-20260209-0007", "커버", "EA"],
  ["MAT-20260209-0008", "단자", "EA"],
  ["MAT-20260209-0009", "라벨", "EA"],
];

const INITIAL_TRANSACTIONS = [
  { id: 1, occurredAt: "2026-02-10 16:05", type: "INBOUND", materialIndex: 5, materialLotNo: "ML-2602101605-6", productLotNo: "-", quantity: 2323, beforeStock: 1, worker: "정유진", from: "입고 대기장", to: "자재 창고 1", note: "정기 자재 입고" },
  { id: 2, occurredAt: "2026-02-10 16:05", type: "INBOUND", materialIndex: 2, materialLotNo: "ML-2602101605-3", productLotNo: "-", quantity: 33, beforeStock: 423, worker: "이준호", from: "입고 대기장", to: "자재 창고 1", note: "정기 자재 입고" },
  ...Array.from({ length: 24 }, (_, index) => {
    const materialIndex = [8, 8, 8, 7, 6, 5, 4, 3, 8, 7, 6, 5, 4, 3, 2, 1, 0][index % 17];
    const quantities = [1, 1, 1, 2, 1, 1, 3, 12, 1, 2, 1, 1, 3, 12, 6, 6, 9];
    const quantity = quantities[index % quantities.length];
    return {
      id: index + 3,
      occurredAt: index < 9 ? "2026-02-10 15:20" : index < 18 ? "2026-02-10 15:19" : "2026-02-10 14:48",
      type: "CONSUMPTION",
      materialIndex,
      materialLotNo: `ML-260208-${String(materialIndex + 1).padStart(4, "0")}-INIT`,
      productLotNo: index < 12 ? "LOT-20260210-005" : "LOT-20260210-004",
      quantity,
      beforeStock: 61 + index * 8,
      worker: index < 18 ? "박서준" : "김하린",
      from: "자재 창고 1",
      to: "생산 라인 2",
      note: index % 5 === 0 ? "긴급 생산 선 투입" : "작업지시 자재 투입",
    };
  }),
];
...
const TypeBadge = styled.span`
  ...
  background: ${({ $type }) => $type === "INBOUND" ? "#e8f8ef" : "#fdecee"};
  font-size: 12px;
  font-weight: 600;
`;
...
const Location = styled.div`
  span { display: block; margin-bottom: 5px; color: #8a929e; font-size: 11px; }
  strong { color: #2b3139; font-size: 13px; }
`;

const Arrow = styled.span`color: #0755d9; font-size: 20px;`;
...
function MaterialTransactionHistory() {
  ...
  const enrichedTransactions = useMemo(() => transactions.map((transaction) => {
    ...
    return { ...transaction, materialCode, materialName, unit, afterStock: transaction.type === "INBOUND" ? transaction.beforeStock + transaction.quantity : transaction.beforeStock - transaction.quantity };
  }), [transactions]);

  const summary = useMemo(() => {
    const inbound = enrichedTransactions.filter((item) => item.type === "INBOUND").reduce((sum, item) => sum + item.quantity, 0);
    const consumption = enrichedTransactions.filter((item) => item.type === "CONSUMPTION").reduce((sum, item) => sum + item.quantity, 0);
    return { inbound, consumption, ratio: inbound > 0 ? ((consumption / inbound) * 100).toFixed(1) : "0.0" };
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return enrichedTransactions.filter((item) => {
      ...
      return (!filters.startDate || date >= filters.startDate)
        ...
        && (!keyword || [item.materialName, item.materialCode, item.materialLotNo, item.productLotNo].some((value) => value.toLowerCase().includes(keyword)));
    });
  ...
  return <>
    <Page>
      <Header>
        ...
        <Description>자재 LOT별 입고와 생산 투입 내역, 재고 증감 및 이동 위치를 조회합니다.</Description>
      </Header>
      ...
      <FilterPanel>
        ...
        <SearchFilterBar
          ...
          endDateLabel="종료일"
          dateWidth={145}
          filters={[{ name: "type", label: "구분", placeholder: "전체 구분", width: 150, options: [
    ...
    {selectedTransaction && <Overlay onClick={() => setSelectedTransaction(null)} />}
    <Drawer $open={Boolean(selectedTransaction)}>
      {selectedTransaction && <>
        <DrawerHeader><DrawerTitle>자재 입출고 이력 상세 조회</DrawerTitle><CloseButton type="button" aria-label="상세 닫기" onClick={() => setSelectedTransaction(null)}><FiX size={22} /></CloseButton></DrawerHeader>
        <DrawerBody>
          <Section>
            <SectionTitle>입출고 정보</SectionTitle>
            <DetailCard><DetailGrid>
              <DetailItem><DetailLabel>구분</DetailLabel><TypeBadge $type={selectedTransaction.type}>{selectedTransaction.type === "INBOUND" ? <FiBox /> : <FiArrowUpCircle />}{typeLabel(selectedTransaction.type)}</TypeBadge></DetailItem>
              <DetailItem><DetailLabel>일시</DetailLabel><DetailValue>{selectedTransaction.occurredAt}</DetailValue></DetailItem>
              <DetailItem><DetailLabel>이동 수량</DetailLabel><DetailValue>{formatNumber(selectedTransaction.quantity)} {selectedTransaction.unit}</DetailValue></DetailItem>
              <DetailItem><DetailLabel>제품 LOT 번호</DetailLabel><DetailValue>{selectedTransaction.productLotNo}</DetailValue></DetailItem>
              <DetailItem $full><DetailLabel>자재 LOT 번호</DetailLabel><DetailValue>{selectedTransaction.materialLotNo}</DetailValue></DetailItem>
              <DetailItem $full><DetailLabel>재고 증감 (Before → After)</DetailLabel><DetailValue>{formatNumber(selectedTransaction.beforeStock)} → {formatNumber(selectedTransaction.afterStock)}</DetailValue></DetailItem>
            </DetailGrid></DetailCard>
          </Section>
          <Section>
            <SectionTitle>이동 경로</SectionTitle>
            <Movement><Location><span>출발 (From)</span><strong>{selectedTransaction.from}</strong></Location><Arrow>→</Arrow><Location><span>도착 (To)</span><strong>{selectedTransaction.to}</strong></Location></Movement>
          </Section>
          <Section>
            <SectionTitle>자재 정보</SectionTitle>
            <DetailCard><DetailGrid><DetailItem $full><DetailLabel>자재코드</DetailLabel><DetailValue>{selectedTransaction.materialCode}</DetailValue></DetailItem><DetailItem $full><DetailLabel>자재명</DetailLabel><DetailValue>{selectedTransaction.materialName}</DetailValue></DetailItem></DetailGrid></DetailCard>
          </Section>
          <Section>
            <SectionTitle>참조 정보</SectionTitle>
            <DetailCard><DetailGrid><DetailItem $full><DetailLabel>담당자</DetailLabel><DetailValue>{selectedTransaction.worker}</DetailValue></DetailItem><DetailItem $full><DetailLabel>비고</DetailLabel><DetailValue>{selectedTransaction.note}</DetailValue></DetailItem></DetailGrid></DetailCard>
          </Section>
        </DrawerBody>
      </>}
    </Drawer>
